...
contract TokenAvgPriceV3 is Initializable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function setDailyPrice(address token, uint256 day, uint256 price) external onlyOwner whenNotPaused {
        require(block.timestamp >= yearStart + (86400 * day) && 
            block.timestamp < (yearStart + (86400 * day) + 1 days), "Can only set current day's price");
